...
class MainApp(tk.Tk):
    ...
    Crea una ventana con un menú desplegable en la parte superior en donde se debe cargar
    una base de datos. A partir de esto, se debe ingresar un rango para los parámetros
    economicos en el cálculo del beneficio de un bloque. Tras seleccionar el rango, se
    procede a realizar las iteraciones.

    Finalmente, desde el menú desplegable, se puede seleccionar la opción
    'Footprint Graph', en donde se puede ver el cambio en las distintas cotas, en base a
    los casos de estudio, siendo el best case el mejor caso de estudio, en donde el
    precio y la recuperación son los maximos, y los costos son los minimos. Por otra
    parte, el worst case será el caso contrario, en donde el precio y la recuperación son
    las menores, y los costos son los mayores.
    
    Se definen también los casos intermedios
    de estudio, siendo el Mid Case (25%) el caso de estudio en el cual se obtiene el 25%
    del valor del best case. Misma lógica para el Mid Case (50%) y el Mid Case (75%).
    """

    WINDOW_HEIGHT = 900
    WINDOW_WIDTH = 1250
    COLOR_LEFT_BG = '#2d3436'
    COLOR_RIGHT_BG = '#636e72'
    COLOR_LETTER = 'white'
    COLOR_ENTRY_DISABLED = 'red'
    ...
    def create_control_variables(self):
        ...
        self.var_dilution = tk.BooleanVar(value = True)
    ...
    def create_menubar(self):
        ...
        self.menu_bar.add_cascade(label = 'View', menu = self.menu_graph)
        self.config(menu = self.menu_bar)


    def widgets_create(self):
        """Crea los widgets de la ventana"""
        self.frame_control = tk.Frame(self, width = self.WINDOW_WIDTH//3, bg = self.COLOR_LEFT_BG)
        self.frame_graph = tk.Frame(self, width = self.WINDOW_WIDTH*2//3, bg = self.COLOR_RIGHT_BG)


        self.frame_combo = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_combo_x = tk.Label(self.frame_combo, text = 'X Coordinate', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_combo_y = tk.Label(self.frame_combo, text = 'Y Coordinate', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_combo_z = tk.Label(self.frame_combo, text = 'Z Coordinate', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_combo_cu = tk.Label(self.frame_combo, text = 'Metal grade', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.frame_graph_buttons = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)
        ...
        self.frame_price = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_price = tk.Label(self.frame_price, text = 'Price (usd/lb)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_price_min = tk.Label(self.frame_price, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_price_max = tk.Label(self.frame_price, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_price_step = tk.Label(self.frame_price, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.frame_minecost = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_minecost = tk.Label(self.frame_minecost, text = 'Mine cost (usd/ton)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_minecost_min = tk.Label(self.frame_minecost, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_minecost_max = tk.Label(self.frame_minecost, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_minecost_step = tk.Label(self.frame_minecost, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.frame_plantcost = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_plantcost = tk.Label(self.frame_plantcost, text = 'Plant cost (usd/ton)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_plantcost_min = tk.Label(self.frame_plantcost, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_plantcost_max = tk.Label(self.frame_plantcost, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_plantcost_step = tk.Label(self.frame_plantcost, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.frame_discountrate = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_discountrate = tk.Label(self.frame_discountrate, text = 'Discount rate (%)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_discountrate_min = tk.Label(self.frame_discountrate, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_discountrate_max = tk.Label(self.frame_discountrate, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_discountrate_step = tk.Label(self.frame_discountrate, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.frame_recovery = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_recovery = tk.Label(self.frame_recovery, text = 'Recovery (%)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_recovery_min = tk.Label(self.frame_recovery, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_recovery_max = tk.Label(self.frame_recovery, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_recovery_step = tk.Label(self.frame_recovery, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.frame_sellcost = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_sellcost = tk.Label(self.frame_sellcost, text = 'Selling cost (usd/ton)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_sellcost_min = tk.Label(self.frame_sellcost, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_sellcost_max = tk.Label(self.frame_sellcost, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_sellcost_step = tk.Label(self.frame_sellcost, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        ...
        self.check_dilution_option = tk.Checkbutton(self.frame_control, variable = self.var_dilution, bg = self.COLOR_LEFT_BG, activebackground = self.COLOR_LEFT_BG, command = self.dilution_refresh)

        self.frame_dilution = tk.Frame(self.frame_control, bg = self.COLOR_LEFT_BG)

        self.label_dilution = tk.Label(self.frame_dilution, text = 'Dilution (%)', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_dilution_min = tk.Label(self.frame_dilution, text = 'Min', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_dilution_max = tk.Label(self.frame_dilution, text = 'Max', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.label_dilution_step = tk.Label(self.frame_dilution, text = 'Step', bg = self.COLOR_LEFT_BG, fg = self.COLOR_LETTER)
        self.entry_dilution_min = tk.Entry(self.frame_dilution, width = 5, disabledbackground = '#787878', justify = 'center', validate = 'key', validatecommand = (self.number_verification, '%P'))
        self.entry_dilution_max = tk.Entry(self.frame_dilution, width = 5, disabledbackground = '#787878', justify = 'center', validate = 'key', validatecommand = (self.number_verification, '%P'))
        self.entry_dilution_step = tk.Entry(self.frame_dilution, width = 5, disabledbackground = '#787878', justify = 'center', validate = 'key', validatecommand = (self.number_verification, '%P'))
        ...
        self.label_progressbar_percentage = tk.Label(self.frame_control, textvariable = self.progressbar_percentage, font = ('Courier New', 8), bg = self.COLOR_LEFT_BG, fg = 'white')
        self.label_eta = tk.Label(self.frame_control, textvariable = self.str_eta, font = ('Courier New', 8), bg = self.COLOR_LEFT_BG, fg = 'white')
    ...
    def widgets_layout(self):
        ...
        self.check_dilution_option.place(x = 50, y = 720)


        self.frame_dilution.pack(pady = 5, padx = 10)

        self.label_dilution.grid(columnspan = 3, row = 0, column = 0, pady = (10, 0), padx = 10)
        self.label_dilution_min.grid(row = 1, column = 0, padx = 10)
        self.label_dilution_max.grid(row = 1, column = 1, padx = 10)
        self.label_dilution_step.grid(row = 1, column = 2, padx = 10)
        self.entry_dilution_min.grid(row = 2, column = 0, padx = 10)
        self.entry_dilution_max.grid(row = 2, column = 1, padx = 10)
        self.entry_dilution_step.grid(row = 2, column = 2, padx = 10)


        self.button_iterate.pack(pady = (45, 5), padx = 10)
        self.progressbar_iterations.pack(padx = 10)
        self.label_progressbar_percentage.place(x = 280, y = 843)
        self.label_eta.pack()
    ...
    def dilution_refresh(self):
        """Esta función activa y desactiva los entries para ingresar los parámetros de la dilución"""
        if self.var_dilution.get():
            self.entry_dilution_min['bg'] = '#ffffff'
            self.entry_dilution_min['cursor'] = 'xterm'
            self.entry_dilution_min['state'] = 'normal'

            self.entry_dilution_max['bg'] = '#ffffff'
            self.entry_dilution_max['cursor'] = 'xterm'
            self.entry_dilution_max['state'] = 'normal'

            self.entry_dilution_step['bg'] = '#ffffff'
            self.entry_dilution_step['cursor'] = 'xterm'
            self.entry_dilution_step['state'] = 'normal'

        else:
            self.entry_dilution_min['cursor'] = 'x_cursor'
            self.entry_dilution_min['state'] = 'disabled'

            self.entry_dilution_max['cursor'] = 'x_cursor'
            self.entry_dilution_max['state'] = 'disabled'

            self.entry_dilution_step['cursor'] = 'x_cursor'
            self.entry_dilution_step['state'] = 'disabled'
    ...
    def list_entries_append(self):
        ...
        if self.var_dilution.get():
            try:
                self.list_entries.append([])
                self.list_entries[6].append(float(self.entry_dilution_min.get()))
                self.list_entries[6].append(float(self.entry_dilution_max.get()))
                self.list_entries[6].append(float(self.entry_dilution_step.get()))
            except ValueError:
                return
    ...
    def iterations(self):
        ...
        if self.var_dilution.get():
            parameters_ranges.append(self.define_list(self.list_entries[6][0], self.list_entries[6][1], self.list_entries[6][2]))
        else:
            parameters_ranges.append([0])
        ...
        for price, m_cost, p_cost, discount, recov, sell_c, dilut in combinations:
            t1 = time()
            saved_data = {
            ...
            df_saved['value'] = self.calculate_block_value(price, sell_c, volume, 2.7, recov, df_saved['grade'], dilut, m_cost, p_cost)
            df_saved['disc_value'] = df_saved['value']/((1 + discount/100) ** df_saved['period'])
            df_saved = self.set_antes_max(df_saved)

            file_name = f'p{price}mc{m_cost}pc{p_cost}d{discount}r{recov}sc{sell_c}dil{dilut}.csv'

            case = self.set_study_case(parameters_ranges, price, m_cost, p_cost, discount, recov, sell_c, dilut)
    ...
    def set_study_case(self, parameters_range:list, price:float, mine_cost:float, plant_cost:float, discount:int, recovery:int, sell_cost:float, dilution:int|float):
        """
    ...
    def calculate_block_value(self, price:int|float, sell_cost:int|float, volume:int|float, density:int|float, recovery:int, grade:int|float, dilution:int|float, mine_cost:int|float, plant_cost:int|float):
        """
        ...
        * income - outcome: Valor del bloque
        ...
        Ejemplo de uso:
            ...
            41,088.78
        """
        income = (price - sell_cost)*volume*density*(recovery/100)*(grade/100)/(1+dilution)*2204.63
...
class AppButton(tk.Frame):
    ...
    def __init__(self, parent, text, bg = '#55efc4', highlightbackground = '#00b894', command = None, *args, **kwargs):
